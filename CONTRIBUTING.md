...
- `scope` 必须是有归属的领域或模块，从现有词表里选：`auth`、`identity`、`projects`、`proofreading`、`onboarding`、`keyboards`、`pdf`、`import`、`profiles`、`rare-chars`、`pagination`、`quality`、`migrations`、`ops`、`deps`、`ci`。词表里没有合适项时，在 PR 描述中说明新 scope 对应哪个目录或领域，再补进本表。
